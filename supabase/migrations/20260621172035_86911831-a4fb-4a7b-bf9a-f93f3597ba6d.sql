...
BEGIN
  ...
  group_cancels AS (
    ...
    WHERE g.cancelled_at IS NOT NULL
      ...
      AND p.hospital_id = current_active_hospital()
      -- Só conta como economia se o motivo for "economia real" (ou se for legado sem motivo classificado).
      AND (g.cancellation_reason IS NULL OR g.cancellation_reason = 'economia_real'::payment_cancellation_reason)
  ),
  item_cancels AS (
    ...
    WHERE pi.is_cancelled = true
      ...
      AND p.hospital_id = current_active_hospital()
      -- Só conta como economia se o motivo for "economia real" (ou se for legado sem motivo classificado).
      AND (pi.cancellation_reason IS NULL OR pi.cancellation_reason = 'economia_real'::payment_cancellation_reason)
  ),
  final_items AS (
