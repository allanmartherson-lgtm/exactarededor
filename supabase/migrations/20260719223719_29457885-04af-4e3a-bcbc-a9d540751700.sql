...
BEGIN
  ...
  eligible AS (
    -- Filtra na origem para reduzir o volume varrido. Só itens com pista
    -- de visita/parecer sobrevivem.
    SELECT
      pi.id,
      pi.payment_id,
  ...
  with_doc AS (
    -- LATERAL LIMIT 1: um lookup por linha, priorizando doctor_id
    -- (chave indexada), depois documento, depois nome normalizado.
    SELECT
      s.*,
    ...
    FROM scoped s
    LEFT JOIN LATERAL (
      SELECT dd.id, dd.full_name, dd.specialties
      FROM public.doctors dd
      WHERE
        (s.doctor_id IS NOT NULL AND dd.id = s.doctor_id)
        OR (s.doctor_id IS NULL
            AND s.doctor_document IS NOT NULL
            AND btrim(s.doctor_document) = btrim(coalesce(dd.crm,'')))
        OR (s.doctor_id IS NULL
            AND s.doctor_document IS NULL
            AND s.doctor_name IS NOT NULL
            AND lower(unaccent(btrim(s.doctor_name))) = lower(unaccent(btrim(coalesce(dd.full_name,'')))))
      ORDER BY (dd.id = s.doctor_id) DESC NULLS LAST
      LIMIT 1
    ) d ON TRUE
  ),
  spec_resolved AS (
