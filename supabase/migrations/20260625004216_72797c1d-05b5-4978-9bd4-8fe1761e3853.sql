...
BEGIN
  ...
  -- POOL é soberano: quando o pagamento é de pool, ignoramos doctor_companies
  -- e distribuímos itens entre as PJs do pool (round-robin estável por médico).
  -- O cálculo do pool faz o rateio financeiro depois.
  IF v_pool_id IS NOT NULL THEN
    SELECT coalesce(array_agg(pp.company_id ORDER BY c.name), ARRAY[]::uuid[])
      INTO v_pool_participants
      FROM pool_participants pp
      JOIN companies c ON c.id = pp.company_id
     WHERE pp.pool_id = v_pool_id;

    IF coalesce(array_length(v_pool_participants, 1), 0) = 0 THEN
      RAISE EXCEPTION 'pool sem PJs participantes cadastradas';
    END IF;
    ...
    SELECT
      ...
      v_pool_participants[
        1 + (abs(hashtext(coalesce(u.doctor_name, u.id::text)))
             % array_length(v_pool_participants, 1))
      ] AS company_id
  ...
  ELSE
    -- Fluxo original: resolve médico e cruza com doctor_companies × participantes
