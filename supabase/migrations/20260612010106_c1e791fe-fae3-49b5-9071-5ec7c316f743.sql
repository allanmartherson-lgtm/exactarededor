...
CREATE OR REPLACE FUNCTION public.clone_rule_to_hospital(_rule_id uuid, _target_hospital_id uuid, _new_name text DEFAULT NULL)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.rule_calculations (
    rule_id, label, calc_method, convenio_percentage, fixed_amount, package_amount,
    bonus_amount, bonus_pct, target_amount, multiplier, deflator_pct, auxiliary_pct,
    aux_first_pct, aux_second_pct, instrumentador_pct, reference_table_id,
    procedure_codes, extras_codes, package_main_code, package_included_codes,
    package_visits_count, package_opinions_count, package_auxiliaries_included,
    package_subtype, time_mode, weekdays, includes_holidays, time_start, time_end,
    elective_mode, apply_access_route, include_auxiliaries, exception_table_ids,
    convenio_ids, sector_ids, agreement_names, agreement_match_mode,
    package_absorbed_calc_id, repasse_pct, target_doctor_id,
    sort_order
  )
  SELECT
    v_new_id, label, calc_method, convenio_percentage, fixed_amount, package_amount,
    bonus_amount, bonus_pct, target_amount, multiplier, deflator_pct, auxiliary_pct,
    aux_first_pct, aux_second_pct, instrumentador_pct, reference_table_id,
    procedure_codes, extras_codes, package_main_code, package_included_codes,
    package_visits_count, package_opinions_count, package_auxiliaries_included,
...
REVOKE ALL ON FUNCTION public.clone_rule_to_hospital(uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.clone_rule_to_hospital(uuid, uuid, text) TO authenticated;
