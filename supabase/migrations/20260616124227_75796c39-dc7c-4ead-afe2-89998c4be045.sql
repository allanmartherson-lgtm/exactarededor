...
CREATE TABLE IF NOT EXISTS public.rule_snapshots (
  ...
  hospital_id uuid,
  reason text NOT NULL CHECK (reason IN ('before_edit','after_save','manual_backup','restore_point')),
...
CREATE INDEX IF NOT EXISTS idx_rule_snapshots_rule_id ON public.rule_snapshots(rule_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_rule_snapshots_hospital ON public.rule_snapshots(hospital_id, created_at DESC);

GRANT SELECT, INSERT ON public.rule_snapshots TO authenticated;
GRANT ALL ON public.rule_snapshots TO service_role;

ALTER TABLE public.rule_snapshots ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admin/diretor leem snapshots no escopo"
...
CREATE POLICY "Admin/diretor criam snapshots no escopo"
...
-- 2) Helper para montar snapshot
CREATE OR REPLACE FUNCTION public.build_rule_snapshot_payload(_rule_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
  ...
  SELECT jsonb_build_object(
    ...
    'calculations', COALESCE(
      (SELECT jsonb_agg(to_jsonb(c.*) ORDER BY c.sort_order)
         FROM public.rule_calculations c
...
-- 3) Atualizar RPC com snapshots + safety checks
CREATE OR REPLACE FUNCTION public.apply_rule_save_with_corrections(
  _rule_data jsonb,
  _calculations jsonb,
  _corrections jsonb,
  _allow_calc_reduction boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_is_update THEN
    ...
    -- BLINDAGEM: bloquear redução silenciosa de cálculos
  ...
  -- Re-sincroniza rule_calculations
  ...
  IF v_incoming_calc_count > 0 THEN
    ...
    -- BLINDAGEM: se enviei N cálculos e nenhum entrou, abortar tudo
    ...
    -- BLINDAGEM: se inseri menos do que enviei, abortar
  ...
  -- BLINDAGEM: snapshot after_save (commit junto, atomicamente)
  INSERT INTO public.rule_snapshots(rule_id, hospital_id, reason, payload, calc_count, actor_id)
  VALUES (
...
-- 4) Função de restauração a partir de um snapshot
CREATE OR REPLACE FUNCTION public.restore_rule_from_snapshot(_snapshot_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  v_actor uuid := auth.uid();
  v_snap public.rule_snapshots%ROWTYPE;
  v_rule_payload jsonb;
  v_calcs_payload jsonb;
  v_restored int := 0;
BEGIN
  IF v_actor IS NULL THEN
    RAISE EXCEPTION 'Não autenticado' USING ERRCODE = '28000';
  ...
  IF NOT (public.has_role(v_actor,'admin'::app_role) OR public.has_role(v_actor,'diretor'::app_role)) THEN
    RAISE EXCEPTION 'Apenas admin ou diretor podem restaurar regras' USING ERRCODE = '42501';
  END IF;

  SELECT * INTO v_snap FROM public.rule_snapshots WHERE id = _snapshot_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Snapshot % não encontrado', _snapshot_id USING ERRCODE = 'P0002';
  END IF;

  IF v_snap.hospital_id IS NOT NULL AND NOT public.hospital_scope_allows(v_snap.hospital_id) THEN
    RAISE EXCEPTION 'Usuário sem acesso à unidade do snapshot' USING ERRCODE = '42501';
  END IF;

  v_rule_payload := v_snap.payload->'rule';
  v_calcs_payload := COALESCE(v_snap.payload->'calculations', '[]'::jsonb);

  -- Snapshot do estado atual antes de sobrescrever
  INSERT INTO public.rule_snapshots(rule_id, hospital_id, reason, payload, calc_count, actor_id)
  VALUES (
    v_snap.rule_id, v_snap.hospital_id, 'restore_point',
    public.build_rule_snapshot_payload(v_snap.rule_id),
    (SELECT count(*) FROM public.rule_calculations WHERE rule_id = v_snap.rule_id),
    v_actor
  );

  -- Sobrescreve cálculos a partir do payload
  DELETE FROM public.rule_calculations WHERE rule_id = v_snap.rule_id;

  IF jsonb_array_length(v_calcs_payload) > 0 THEN
    INSERT INTO public.rule_calculations
    SELECT (jsonb_populate_record(
      NULL::public.rule_calculations,
      (c.value - 'id' - 'created_at' - 'updated_at')
        || jsonb_build_object(
             'id', gen_random_uuid(),
             'rule_id', v_snap.rule_id,
             'hospital_id', v_snap.hospital_id,
             'created_at', to_jsonb(now()),
             'updated_at', to_jsonb(now())
           )
    )).*
    FROM jsonb_array_elements(v_calcs_payload) AS c(value);

    GET DIAGNOSTICS v_restored = ROW_COUNT;
  END IF;

  INSERT INTO public.audit_log(actor_id, entity_type, entity_id, action, diff, hospital_id)
  VALUES (v_actor, 'rule', v_snap.rule_id, 'restore_from_snapshot',
          jsonb_build_object('snapshot_id', _snapshot_id, 'reason', v_snap.reason, 'calc_count', v_restored),
          v_snap.hospital_id);

  RETURN jsonb_build_object(
    'ok', true,
    'rule_id', v_snap.rule_id,
    'calculations_restored', v_restored
  );
END;
$$;
