...
CREATE OR REPLACE FUNCTION public.apply_zeev_bulk_manual(
  ...
  _custom_value numeric DEFAULT NULL
)
...
BEGIN
  ...
  END IF;
  IF _strategy NOT IN ('procedure','expected','custom') THEN
    RAISE EXCEPTION 'invalid value_strategy: %', _strategy;
  END IF;
    ...
    UPDATE public.payment_items pi
       SET manual_intervention_reason_id = _reason_id,
           ...
           expected_amount = CASE
             ...
             ELSE pi.expected_amount  -- 'expected' → não mexe
           END,
           gross_amount = CASE
             WHEN _strategy = 'procedure' THEN COALESCE(pi.procedure_amount, pi.gross_amount)
             WHEN _strategy = 'custom'    THEN _custom_value
             ELSE pi.gross_amount     -- 'expected' → não mexe
           END,
...
REVOKE EXECUTE ON FUNCTION public.apply_zeev_bulk_manual(uuid[], uuid, text, text, text, text, numeric) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.apply_zeev_bulk_manual(uuid[], uuid, text, text, text, text, numeric) TO authenticated;
