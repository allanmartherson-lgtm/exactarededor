...
DECLARE
  ...
  v_err text;
  v_bp numeric; v_br numeric; v_bt numeric;
BEGIN
  ...
  SELECT bruto_pedido_total, bruto_regra_total INTO v_bp, v_br FROM public.vw_group_rule_totals WHERE group_id = v_group_normal;

  RETURN QUERY SELECT 'debug_setup'::text, true,
    format('group.bruto_total=%s view.bruto_pedido=%s view.bruto_regra=%s', v_bt, v_bp, v_br);
