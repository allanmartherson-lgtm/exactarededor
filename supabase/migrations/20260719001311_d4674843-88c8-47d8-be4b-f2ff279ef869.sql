...
CREATE OR REPLACE FUNCTION public.get_batch_composition(
  p_processing_month date DEFAULT NULL,
...
BEGIN
  ...
  current_month AS (
    SELECT
      ...
      max(payment_id) as cur_pid,
      max(reference) as cur_ref
    FROM normalized
    WHERE comp = v_proc_month
      AND pattern != ''
    GROUP BY pattern
...
GRANT EXECUTE ON FUNCTION public.get_batch_composition(date, integer) TO authenticated;
