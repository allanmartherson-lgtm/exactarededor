...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      ...
      (SELECT da.doctor_id FROM doctor_aliases da
        WHERE normalize_alias(da.alias) = normalize_alias(u.doctor_name)
        LIMIT 1),
      (SELECT d.id FROM doctors d
  ...
  UPDATE payment_unmatched_items u
     SET status = 'linked',
         ...
         resolved_company_id = m.company_id
    FROM _mapped m
   WHERE u.id = m.unmatched_id
     AND m.company_id IS NOT NULL;

  SELECT count(*)::int INTO v_unresolved
    FROM _mapped WHERE company_id IS NULL;

  SELECT COALESCE(array_agg(DISTINCT company_id), ARRAY[]::uuid[])
    INTO v_companies
    FROM _mapped WHERE company_id IS NOT NULL;

  RETURN QUERY SELECT v_linked, v_unresolved, v_companies;
END;
$$;

REVOKE EXECUTE ON FUNCTION public.distribute_unmatched_items_by_doctor(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.distribute_unmatched_items_by_doctor(uuid, text) TO authenticated;