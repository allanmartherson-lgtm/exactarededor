...
BEGIN
  ...
  -- Correções de valor feitas pelo analista durante a análise
  ...
  analyst_final AS (
    SELECT ae.item_id, ae.payment_id, ae.obs_id, ae.author_id,
           'analista'::text AS role, ae.obs_at,
           ae.obs_at AS acatado_at,
           ae.old_val AS valor_regra,
           ae.new_val AS valor_pago_final,
    ...
    LEFT JOIN public.payment_company_groups g
      ON g.payment_id = ae.payment_id AND g.company_id = pi.company_id
  ...
  -- Cancelamentos de empresa/grupo (qualquer item dentro vira economia)
  group_cancels AS (
    SELECT pi.id AS item_id, g.payment_id,
           g.id::text AS obs_id,
           g.cancelled_by AS author_id,
           'cancelamento_empresa'::text AS role,
           g.cancelled_at AS obs_at,
           g.cancelled_at AS acatado_at,
  ...
  -- Cancelamentos de item individual (exclui itens cujo grupo também foi cancelado no período)
  item_cancels AS (
    SELECT pi.id AS item_id, pi.payment_id,
           pi.id::text AS obs_id,
           pi.cancelled_by AS author_id,
           'cancelamento_item'::text AS role,
           pi.cancelled_at AS obs_at,
           pi.cancelled_at AS acatado_at,
           COALESCE(pi.gross_amount, 0) AS valor_regra,
           0::numeric AS valor_pago_final,
           COALESCE(pi.gross_amount, 0) AS delta
  ...
  final_items AS (
    ...
    UNION ALL
    SELECT * FROM analyst_final
    UNION ALL
    SELECT * FROM group_cancels
    UNION ALL
    SELECT * FROM item_cancels
  ),
  summary AS (
