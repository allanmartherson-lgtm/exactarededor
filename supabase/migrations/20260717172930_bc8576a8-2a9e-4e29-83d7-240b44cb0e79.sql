...
BEGIN
  ...
  direct_match AS (
    ...
    FROM candidates c
    WHERE trim(c.norm_c) <> ''
      AND EXISTS (SELECT 1 FROM item_norms i WHERE i.n = trim(c.norm_c))
  ),
  alias_match AS (
    SELECT c.original
    FROM candidates c
    WHERE p_mode = 'procedimento'
      AND EXISTS (
        ...
        JOIN item_norms i
          ON i.n = regexp_replace(
                     regexp_replace(lower(unaccent(pa.canonical_name)),
                       '[^a-z0-9 ]+', ' ', 'g'),
                     '\s+', ' ', 'g')
        WHERE pa.hospital_id = p_hospital_id
          AND pa.alias_normalized = trim(c.norm_c)
      )
    UNION
    SELECT c.original
    FROM candidates c
    WHERE p_mode = 'medico'
      AND EXISTS (
        ...
        JOIN item_norms i
          ON i.n = regexp_replace(
                     regexp_replace(lower(unaccent(d.full_name)),
                       '[^a-z0-9 ]+', ' ', 'g'),
                     '\s+', ' ', 'g')
        WHERE da.alias_normalized = trim(c.norm_c)
      )
  )
...
REVOKE ALL ON FUNCTION public.get_simulator_matched_names(uuid,integer,text,text[]) FROM public;
GRANT EXECUTE ON FUNCTION public.get_simulator_matched_names(uuid,integer,text,text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_simulator_matched_names(uuid,integer,text,text[]) TO service_role;
