...
BEGIN
  ...
  SELECT
    ...
    -- Autor operacional apenas. NUNCA cair para approved_by (diretor aprovador
    -- não é interventor). Sem autor operacional => NULL (aparece como "Sistema").
  ...
  -- Glosa por PJ: autor = quem aplicou/confirmou. Sem operador => NULL (Sistema).
  INSERT INTO public.intervention_ledger (
    payment_id, item_id, hospital_id, company_id, company_name,
    doctor_name, procedure_code, procedure_name,
    valor_regra, valor_pago_final, delta,
...
-- Rematerializa lotes aprovados nos últimos 90 dias para reescrever autoria.
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
