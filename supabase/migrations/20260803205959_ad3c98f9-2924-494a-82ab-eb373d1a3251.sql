CREATE OR REPLACE FUNCTION public.apply_calc_duplicity_resolution(_item_id uuid, _chosen_calc_id uuid, _justification text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Escopo multi-tenant: item precisa pertencer ao hospital ativo (roles globais passam)
  SELECT pi.hospital_id, true INTO v_item_hospital, v_item_exists
  ...
  PERFORM public.assert_hospital_access(v_item_hospital);
