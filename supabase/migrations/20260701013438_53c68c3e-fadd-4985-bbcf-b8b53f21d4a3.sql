...
BEGIN
  ...
  SELECT
    ...
    x.item_type_id, x.item_type_source
...
BEGIN
  ...
  INSERT INTO public.payment_unmatched_items (
    payment_id, source_file, raw_company_name, match_score, match_suggestion_id,
    match_suggestion_name, hospital_id, doctor_name, doctor_document, doctor_email,
    description, gross_amount, attendance_number, procedure_code, procedure_name,
    access_route, doctor_role, agreement_text, specialty, procedure_amount,
    quantity, procedure_date, procedure_date_has_time, patient_name, sector,
    attendance_character, raw_data, tipo_linha, convenio_value_totalized
  )
  SELECT
    _payment_id, x.source_file, x.raw_company_name, COALESCE(x.match_score, 0),
    x.match_suggestion_id, x.match_suggestion_name, x.hospital_id, x.doctor_name,
    x.doctor_document, x.doctor_email, x.description, x.gross_amount,
    x.attendance_number, x.procedure_code, x.procedure_name, x.access_route,
    x.doctor_role, x.agreement_text, x.specialty, x.procedure_amount,
    COALESCE(x.quantity, 1), x.procedure_date, COALESCE(x.procedure_date_has_time, false),
    x.patient_name, x.sector, x.attendance_character, x.raw_data, x.tipo_linha,
