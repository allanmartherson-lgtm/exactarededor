...
CREATE OR REPLACE FUNCTION public.get_overlap_audit(
  ...
  p_excluded_specs text[] DEFAULT ARRAY[]::text[]
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  eligible AS (
    SELECT
      ...
      CASE
        ...
        WHEN pi.procedure_name !~ '^\d' AND regexp_replace(coalesce(pi.procedure_code,''), '\D', '', 'g') = '10103082' THEN 'parecer'
        WHEN pi.procedure_name !~ '^\d' AND regexp_replace(coalesce(pi.procedure_code,''), '\D', '', 'g') = '10103015' THEN 'parecer'
        WHEN pi.procedure_name !~ '^\d' AND regexp_replace(coalesce(pi.procedure_code,''), '\D', '', 'g') = '10102019' THEN 'visita'
        WHEN pi.procedure_name !~ '^\d' AND regexp_replace(coalesce(pi.procedure_code,''), '\D', '', 'g') = '10102027' THEN 'visita'
        ELSE NULL
      END AS item_kind
    ...
    WHERE pi.hospital_id = v_hospital
      ...
      AND (
        ...
        OR (pi.procedure_name !~ '^\d' AND regexp_replace(coalesce(pi.procedure_code,''), '\D', '', 'g') IN
  ...
  -- Somente itens que ainda precisam resolver médico (sem specialty gravada)
  needs_doc AS (
    SELECT DISTINCT
      doctor_id,
  ...
  -- Resolve doctor_id uma vez por chave (não por linha)
  key_resolved AS (
    SELECT
      nd.doctor_id,
  ...
  with_specs AS (
    SELECT
      ...
      -- Se pi.specialty está preenchida, usar como array; senão pegar do doctor
      CASE
