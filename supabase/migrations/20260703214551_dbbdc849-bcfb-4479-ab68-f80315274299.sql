...
begin
  ...
  if v_item_date is not null and (v_item_date < v_period_start or v_item_date > v_period_end) then
...
drop trigger if exists trg_enforce_tvr_reconciliation_item_scope on public.retroactive_reconciliation_items;
create trigger trg_enforce_tvr_reconciliation_item_scope
before insert or update of source, raw, reconciliation_id, procedure_date
on public.retroactive_reconciliation_items
for each row
execute function public.enforce_tvr_reconciliation_item_scope();