...
BEGIN
  ...
  -- Baseline = PAGO PRÉ-INTERVENÇÃO.
  -- Prioridade: gross_amount_original (quando gravado com valor real) →
  --             gross_amount_at_time da 1ª versão da análise (snapshot) →
  --             gross_amount atual (fallback).
  -- Delta positivo = economia (hospital paga menos); negativo = perda.
  ...
  SELECT
    ...
    COALESCE(
      ...
      (SELECT v.gross_amount_at_time
         FROM public.ai_analysis_versions v
        WHERE v.item_id = pi.id
          AND v.gross_amount_at_time IS NOT NULL
