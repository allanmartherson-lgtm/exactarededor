...
BEGIN
  ...
  final_intervention AS (
    SELECT item_id, payment_id, obs_id::text AS obs_id, author_id, role, obs_at, acatado_at,
  ...
  analyst_final AS (
    SELECT ae.item_id, ae.payment_id, ae.obs_id::text AS obs_id, ae.author_id,
  ...
  group_cancels AS (
    SELECT pi.id AS item_id, g.payment_id, g.id::text AS obs_id,
           g.cancelled_by AS author_id, 'cancelamento_empresa'::text AS role,
  ...
  item_cancels AS (
    SELECT pi.id AS item_id, pi.payment_id, pi.id::text AS obs_id,
           pi.cancelled_by AS author_id, 'cancelamento_item'::text AS role,
