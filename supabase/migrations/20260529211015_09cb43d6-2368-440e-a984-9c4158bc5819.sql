...
BEGIN
  ...
  IF COALESCE(v_empresa_tem_pool, false) THEN
    -- liquido total da empresa (snapshot uniforme por empresa/competência — usamos MAX)
    ...
    -- concatena itens de origem do rateio, deduplicando por id
    WITH origin AS (
    ...
    -- quota consolidada (uma por pool/empresa — pega a primeira não nula)
    WITH quotas AS (
