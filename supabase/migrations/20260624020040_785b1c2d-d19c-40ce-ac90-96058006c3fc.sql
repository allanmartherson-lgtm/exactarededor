...
-- Função de regressão: garante que o trigger check_group_reconciliation_gate
-- sempre PULA o gate quando import_mode='historico' e BLOQUEIA em modo normal
-- quando a divergência excede a tolerância.
...
BEGIN
  ...
  -- Setup: dois pagamentos (um histórico, um normal) com grupos em revisao_analista
  INSERT INTO public.payments(id, reference, created_by, hospital_id, import_mode, status)
    VALUES (v_payment_historico, v_ref || '-H', v_user_id, v_hospital_id, 'historico', 'pago'),
           (v_payment_normal,    v_ref || '-N', v_user_id, v_hospital_id, 'normal',    'em_analise');
  ...
  -- TESTE 1: histórico → trigger DEVE permitir transição para 'pago' sem erro
  BEGIN
  ...
  -- TESTE 2: normal com divergência grande → trigger DEVE bloquear
  -- Inserir item com bruto_pedido alto e nenhum rule_calculation (bruto_regra=0)
  -- A view vw_group_rule_totals soma bruto_pedido a partir de payment_items.
  -- Se não houver itens, ambos os totais ficam 0 e o gate não bloqueia.
  -- Forçamos via override snapshot inverso: criamos um payment_item simulado.
...
EXCEPTION WHEN OTHERS THEN
  -- Garantir cleanup mesmo em erro inesperado
  DELETE FROM public.payment_items WHERE payment_id IN (v_payment_historico, v_payment_normal);
  DELETE FROM public.payment_company_groups WHERE id IN (v_group_historico, v_group_normal);
  DELETE FROM public.payments WHERE id IN (v_payment_historico, v_payment_normal);
  RAISE;
...
COMMENT ON FUNCTION public.test_group_reconciliation_gate() IS
  'Regressão do trigger check_group_reconciliation_gate: histórico pula, normal bloqueia divergência. Rodar: SELECT * FROM public.test_group_reconciliation_gate();';
