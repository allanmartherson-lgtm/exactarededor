...
BEGIN
  ...
  SELECT jsonb_build_object(
    -- Usa o mesmo valor exibido na lista (líquido, com fallback para bruto/total)
    -- para evitar divergência visual quando total_amount foi gravado com bruto+líquido
    -- somados ou com inconsistência histórica.
    'totalOpen', COALESCE((SELECT sum(COALESCE(liquido_total, bruto_total, total_amount)) FROM open_base), 0),
