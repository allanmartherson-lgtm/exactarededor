...
BEGIN
  ...
  -- bruto_total alto + nenhum payment_item (bruto_regra=0) → divergência 100% no view
  INSERT INTO public.payment_company_groups(id, payment_id, hospital_id, company_name, status, bruto_total)
    VALUES (v_group_historico, v_payment_historico, v_hospital_id, 'TEST HISTORICO', 'revisao_analista', 100000),
           (v_group_normal,    v_payment_normal,    v_hospital_id, 'TEST NORMAL',    'revisao_analista', 100000);

  -- TESTE 1: histórico DEVE permitir transição para 'pago' apesar da divergência
  BEGIN
  ...
  -- TESTE 2: normal com mesma divergência DEVE ser bloqueado
  BEGIN
  ...
  IF v_caught_normal THEN
    RETURN QUERY SELECT 'normal_blocks_on_divergence'::text, true,
      ('trigger bloqueou como esperado: ' || left(v_err, 140))::text;
  ELSE
