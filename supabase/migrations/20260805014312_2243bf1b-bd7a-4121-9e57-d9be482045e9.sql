...
BEGIN
  ...
  -- Apelido cadastrado pelo analista: só resolve quando aponta para um único médico.
  by_alias AS (
    SELECT n.norm_name, min(da.doctor_id) AS doctor_id
    FROM names n
    JOIN public.doctor_aliases da ON da.alias_normalized = n.norm_name
    GROUP BY n.norm_name
    HAVING count(DISTINCT da.doctor_id) = 1
  ),
  -- Fallback: nome exato do cadastro, também exigindo unicidade.
  by_name AS (
    SELECT n.norm_name, min(d.id) AS doctor_id
    FROM names n
