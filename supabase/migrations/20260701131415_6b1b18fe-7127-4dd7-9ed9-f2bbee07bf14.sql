...
BEGIN
    ...
    -- Pega qualquer company existente (não tem hospital_id — usada só como FK)
    ...
    IF v_co_placeholder IS NOT NULL THEN
      ...
      BEGIN
        INSERT INTO public.payment_company_groups(id,hospital_id,payment_id,company_id,created_at,updated_at) VALUES
          (v_pcg_a,hosp_a,v_pay_a,v_co_placeholder,now(),now()),
          (v_pcg_b,hosp_b,v_pay_b,v_co_placeholder,now(),now());
      ...
      BEGIN
        INSERT INTO public.company_threads(id,hospital_id,company_id,subject,status,created_at,updated_at) VALUES
          (v_thr_a,hosp_a,v_co_placeholder,'T A','open',now(),now()),
          (v_thr_b,hosp_b,v_co_placeholder,'T B','open',now(),now());
      ...
      BEGIN
        INSERT INTO public.invoices(id,hospital_id,company_id,competence_month,created_at,updated_at) VALUES
          (v_inv_a,hosp_a,v_co_placeholder,date_trunc('month',now())::date,now(),now()),
          (v_inv_b,hosp_b,v_co_placeholder,date_trunc('month',now())::date,now(),now());
    ...
    FOR r IN SELECT unnest(ARRAY['payments','glosa_batches','company_threads','pendencias']) AS tname
    LOOP
      BEGIN
