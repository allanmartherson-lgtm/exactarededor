...
BEGIN
  ...
  -- Glosa por PJ: autor = quem aplicou/confirmou (operacional), não o diretor aprovador.
  INSERT INTO public.intervention_ledger (
    payment_id, item_id, hospital_id, company_id, company_name,
    doctor_name, procedure_code, procedure_name,
    valor_regra, valor_pago_final, delta,
...
BEGIN
  FOR r IN
    SELECT DISTINCT payment_id
      FROM public.intervention_ledger
     WHERE fonte = 'glosa_pj'
  LOOP
    PERFORM public.materialize_intervention_ledger(r.payment_id);
  END LOOP;
END$$;
