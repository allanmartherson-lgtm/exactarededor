...
BEGIN
  ...
  -- Sem isto, quando chamada via service_role auth.uid() é NULL e triggers
  -- que validam autenticação (assert_hospital_access, seg-of-duties, etc.)
  -- disparam 'not_authenticated' na cascata de DELETE/UPDATE.
  IF _actor_id IS NOT NULL THEN
    PERFORM set_config(
      'request.jwt.claims',
      json_build_object('sub', _actor_id::text, 'role', 'authenticated')::text,
      true
    );
  END IF;
...
REVOKE ALL ON FUNCTION public.admin_clear_company_items(uuid, text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_clear_company_items(uuid, text, uuid) TO service_role;