...
BEGIN
    -- Marca chamada como test-context para logs (opcional)
    kind := 'info'; message := 'RLS hospital isolation test — starting'; RETURN NEXT;

    -- Setup hospitais
    INSERT INTO public.hospitals(id, name, slug, cnpj, state_uf, active, created_at)
    VALUES
    ...
    INSERT INTO public.user_hospitals(user_id, hospital_id) VALUES (user_a, hosp_a), (user_b, hosp_b);
    ...
    EXCEPTION WHEN OTHERS THEN
      kind := 'seed_skip'; message := 'payment_company_groups: '||SQLERRM; RETURN NEXT;
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      kind := 'seed_skip'; message := 'glosa_batches: '||SQLERRM; RETURN NEXT;
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      kind := 'seed_skip'; message := 'company_threads: '||SQLERRM; RETURN NEXT;
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      kind := 'seed_skip'; message := 'invoices: '||SQLERRM; RETURN NEXT;
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      kind := 'seed_skip'; message := 'pendencias: '||SQLERRM; RETURN NEXT;
    END;
    ...
    -- hospitals: user A só enxerga o próprio
    SELECT count(*) INTO v_count FROM public.hospitals WHERE id IN (hosp_a, hosp_b);
    IF v_count <> 1 THEN
      v_failed := true;
      kind := 'FAIL'; message := format('hospitals: user A viu %s hospitais de teste (esperado 1)', v_count); RETURN NEXT;
    ELSE
      kind := 'pass'; message := 'hospitals: user A vê apenas o próprio'; RETURN NEXT;
    END IF;

    FOR r IN
      SELECT unnest(ARRAY[
        'payments','payment_company_groups',
        'glosa_batches',
        'company_threads',
        'invoices','pendencias',
    ...
    LOOP
      BEGIN
        ...
        ELSE
          kind := 'pass'; message := format('%s: 0 vazamento de hosp B', r.tname); RETURN NEXT;
        END IF;
      EXCEPTION WHEN OTHERS THEN
    ...
    -- Cross-check como user B
    PERFORM set_config('request.jwt.claims',
        json_build_object('sub', user_b::text, 'role','authenticated','aud','authenticated')::text, true);
    EXECUTE 'SET LOCAL ROLE authenticated';

    FOR r IN
      SELECT unnest(ARRAY['payments','glosa_batches','company_threads','pendencias','doctors','companies']) AS tname
    LOOP
      BEGIN
    ...
    IF v_failed THEN
      ...
      RAISE EXCEPTION 'RLS hospital isolation test FAILED (see result rows)';
    ELSE
...
EXCEPTION WHEN OTHERS THEN
    v_err_msg := SQLERRM;
    -- Cleanup best-effort mesmo em erro
    ...
    BEGIN DELETE FROM public.user_active_hospital WHERE user_id  IN (user_a, user_b); EXCEPTION WHEN OTHERS THEN NULL; END;
    BEGIN DELETE FROM public.user_roles           WHERE user_id  IN (user_a, user_b); EXCEPTION WHEN OTHERS THEN NULL; END;
    BEGIN DELETE FROM public.user_hospitals       WHERE user_id  IN (user_a, user_b); EXCEPTION WHEN OTHERS THEN NULL; END;
...
COMMENT ON FUNCTION public.run_rls_hospital_isolation_test() IS
'Teste automatizado de isolamento por hospital. Cria 2 usuários/hospitais fake, valida RLS em tabelas prioritárias e faz leak scan em toda tabela com hospital_id. Cleanup automático. Rodar: SELECT * FROM public.run_rls_hospital_isolation_test(); Exige service_role.';
