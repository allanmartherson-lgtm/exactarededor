...
BEGIN
    FOR pass IN 1..2 LOOP
      ...
      LOOP
        BEGIN
          EXECUTE format('DELETE FROM public.%I WHERE hospital_id IN ($1,$2)', r.table_name)
            USING _hosp_a, _hosp_b;
        EXCEPTION WHEN OTHERS THEN NULL;
        END;
      END LOOP;
    END LOOP;

    BEGIN DELETE FROM public.audit_log WHERE actor_id IN (_user_a,_user_b); EXCEPTION WHEN OTHERS THEN NULL; END;
    ...
    BEGIN DELETE FROM public.profiles       WHERE id IN (_user_a,_user_b); EXCEPTION WHEN OTHERS THEN NULL; END;
...
BEGIN
    ...
    IF v_co IS NOT NULL THEN
      ...
      EXCEPTION WHEN OTHERS THEN NULL; END;
      BEGIN
        INSERT INTO public.payment_items(id,hospital_id,payment_id,company_id,doctor_name,created_at,updated_at) VALUES
          (gen_random_uuid(),hosp_a,v_pay_a,v_co,'RLS TEST DOC A',now(),now()),
          (v_item_b,hosp_b,v_pay_b,v_co,'RLS TEST DOC B',now(),now());
...
BEGIN
  FOR h IN SELECT id FROM public.hospitals WHERE name LIKE 'RLS-TEST-%' LOOP
    PERFORM public.rls_test_cleanup(h, h, '00000000-0000-0000-0000-000000000000'::uuid, '00000000-0000-0000-0000-000000000000'::uuid);
  END LOOP;
  FOR u IN SELECT id FROM auth.users WHERE email LIKE 'rls-a-%@test.local' OR email LIKE 'rls-b-%@test.local' LOOP
    DELETE FROM public.audit_log WHERE actor_id = u;
    DELETE FROM public.user_active_hospital WHERE user_id = u;
    DELETE FROM public.user_roles WHERE user_id = u;
    DELETE FROM public.user_hospitals WHERE user_id = u;
