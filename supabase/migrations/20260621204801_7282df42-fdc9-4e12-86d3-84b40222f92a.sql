...
-- Reprocessa lotes em modo padrão que possam estar com status travado pela regra antiga.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    SELECT id FROM public.payments
    WHERE analysis_mode = 'padrao'
      AND status IN ('revisao_analista','devolvido_analista','aguardando_validacao','aguardando_aprovacao')
  LOOP
