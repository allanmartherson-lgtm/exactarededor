...
-- 1) Enum payment_track
DO $$ BEGIN
  CREATE TYPE public.payment_track AS ENUM ('prioritario','habitual');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 2) Coluna em payments (nullable = "não classificado")
ALTER TABLE public.payments
  ADD COLUMN IF NOT EXISTS payment_track public.payment_track;

COMMENT ON COLUMN public.payments.payment_track IS
  'Trilha comercial de pagamento. Não afeta cálculos — apenas fluxo (prioritários pagam antes) e segmentação em relatórios.';

CREATE INDEX IF NOT EXISTS payments_payment_track_idx
  ON public.payments (payment_track) WHERE payment_track IS NOT NULL;

-- 3) Atualiza RPC get_payment_pivot para comparar trilha-com-trilha quando o lote tem trilha definida
CREATE OR REPLACE FUNCTION public.get_payment_pivot(
  ...
  p_track text DEFAULT NULL::text  -- 'prioritario' | 'habitual' | NULL = sem filtro de trilha
)
...
begin
  ...
  -- Filtro de trilha: o param explícito (p_track) sobrescreve o do lote.
  -- Se p_track for 'todos' tratamos como NULL (sem filtro).
