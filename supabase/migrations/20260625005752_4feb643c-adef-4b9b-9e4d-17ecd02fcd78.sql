-- 1. Coluna de sinalização
ALTER TABLE public.payment_items
  ADD COLUMN IF NOT EXISTS is_pool_item boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_payment_items_pool ON public.payment_items(payment_id) WHERE is_pool_item = true;
...
BEGIN
  ...
  IF v_pool_id IS NOT NULL THEN
    ...
    -- company_id é opcional em pool; mantém o que veio
  ELSE
...
DROP TRIGGER IF EXISTS trg_enforce_pool_item_consistency ON public.payment_items;
CREATE TRIGGER trg_enforce_pool_item_consistency
  BEFORE INSERT OR UPDATE OF payment_id, company_id ON public.payment_items
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_pool_item_consistency();

-- 3. Backfill: marca itens existentes de lotes pool como is_pool_item
UPDATE public.payment_items pi
   SET is_pool_item = true
  FROM public.payments p
 WHERE pi.payment_id = p.id
   AND p.pool_id IS NOT NULL
   AND pi.is_pool_item = false;

-- 4. Reescrita da função de distribuição: pool deixa item sem PJ (coletivo)
...
BEGIN
  ...
  -- POOL: itens viram coletivos do pool (company_id NULL, is_pool_item true)
  IF v_pool_id IS NOT NULL THEN
    WITH ins AS (
  ...
  -- Fluxo padrão (lote comum): resolve médico → PJ via doctor_companies
  ...
  SELECT count(*)::int INTO v_unresolved FROM _mapped m WHERE m.company_id IS NULL;
...
-- 5. Reverter os itens que round-robineei no lote 07d999fc (transformar em pool-items sem dono)
UPDATE public.payment_items
   SET company_id = NULL, company_name = NULL, is_pool_item = true
 WHERE payment_id = '07d999fc-587e-4d78-a2ba-38ab4f7c240f'
   AND is_pool_item = true;