CREATE OR REPLACE FUNCTION public.apply_rule_save_with_corrections(_rule_data jsonb, _calculations jsonb, _corrections jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF jsonb_array_length(_calculations) > 0 THEN
    -- IMPORTANTE: jsonb_populate_record materializa TODAS as colunas explicitamente,
    -- então DEFAULT now() de created_at/updated_at não é aplicado pelo INSERT.
    -- Precisamos injetar created_at/updated_at = now() no jsonb antes do populate.
