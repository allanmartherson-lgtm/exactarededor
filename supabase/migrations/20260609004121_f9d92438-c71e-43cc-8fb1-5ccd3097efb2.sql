-- Padroniza a máquina de estados: cancel_by_reconciliation só atua na ETAPA DE ANÁLISE.
-- Status válidos (allowlist explícito) — qualquer status fora dessa lista é rejeitado.
-- A função nunca consulta a tabela `invoices` nem usa status de NF.
...
DECLARE
  ...
  v_target_ids uuid[];
  -- Allowlist canônico da ETAPA DE ANÁLISE (antes do ciclo de NF).
  -- Mantenha em sincronia com src/lib/paymentFlow.ts (ANALYST_*_STATUSES).
...
BEGIN
  ...
  -- GATE da etapa de análise: allowlist positivo (não usa status de NF).
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'payment_status_at_action', v_payment_status,
      'scope', p_scope,
      'reason', p_reason,
      'note', p_note,
