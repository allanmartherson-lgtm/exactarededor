...
// ---- Mocks de módulos (precisam vir antes do import do hook) ----
...
// Mock supabase: captura handlers em handlerMap[`${table}:${event}`]
type Handler = (payload: { new: unknown; old?: unknown }) => void;
const handlerMap = new Map<string, Handler>();

const channelObj = {
  on: vi.fn((_evt: string, opts: { table: string; event: string }, cb: Handler) => {
...
vi.mock("@/contexts/AuthContext", () => ({
  useAuth: () => ({
    user: { id: "user-1" },
    hasRole: (r: AppRole) => mockRoles.includes(r),
  }),
}));

// Agora podemos importar o hook
...
describe("useQueueNotifications — gating por papel", () => {
  ...
  describe("nova pendência do prestador", () => {
    it("notifica analista", async () => {
      ...
      // handler tem await em supabase.from(companies); aguarda microtask
