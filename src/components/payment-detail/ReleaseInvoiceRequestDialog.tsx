...
export const ReleaseInvoiceRequestDialog = ({ open, onOpenChange, paymentId, group, onSuccess }: Props) => {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (cancelled) return;
        setEmails(dedupEmails((data?.invoice_emails as string[] | null) ?? []));
        setLoading(false);
      });
    return () => {
      cancelled = true;
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      // 1. Salva e-mails na empresa (se houver company_id)
      if (group.company_id) {
      ...
      // 3. Atualiza status do grupo (trigger recalcula o lote)
