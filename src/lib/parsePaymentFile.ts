...
export const extractCompanyFromFilename = (filename: string): string => {
  ...
  name = name.replace(/\s*[-_]\s*(centro\s*cirurgico|cc|hemodin[âa]mica|consultas?|parecer(es)?|ambulatorial|visitas?|cirurgi[ao]s?|ambulat[oó]rio|uti|enfermaria|interna[cç][aã]o)\b.*$/i, "");
  // Também remove setor colado sem hífen depois de "LTDA/SA" (ex.: Empresa Ltda Centro Cirurgico)
  name = name.replace(/\b(ltda|eireli|me|epp|s\.?a\.?)\s+(centro\s*cirurgico|cc|hemodin[âa]mica|consultas?|parecer(es)?|ambulatorial|visitas?|cirurgi[ao]s?|ambulat[oó]rio|uti|enfermaria|interna[cç][aã]o)\b.*$/i, "$1");
